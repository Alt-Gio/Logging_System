...
export default function LogbookPage() {
  ...
  // ── OTP helpers ──────────────────────────────────────────────────────────────
  const sendOtp = async (contactOverride?: string) => {
    const contact = (contactOverride ?? otpContact).trim()
    if (!contact) return
    setOtpContact(contact)
    ...
    <div className="min-h-screen" style={{position:"relative",zIndex:1}}>
      ...
      <main className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="glass rounded-2xl shadow-sm border border-gray-100 p-5 space-y-4">
          ...
          {/* ── Contact info + OTP verification (PC users) ── */}
          {hasPC && (
            <div className={`rounded-2xl border-2 overflow-hidden transition-colors ${
              otpVerified ? 'border-green-300' : 'border-blue-200'
            }`}>
              {/* Header */}
              <div className={`flex items-start gap-3 px-4 py-3 transition-colors ${
                otpVerified ? 'bg-green-600' : 'bg-[var(--dict-blue)]'
              }`}>
                <span className="text-xl flex-shrink-0 mt-0.5">{otpVerified ? '✅' : '📧'}</span>
                <div>
                  <p className="font-bold text-white text-sm">
                    {otpVerified ? 'Identity Verified' : 'Contact Information Required'}
                  </p>
                  <p className="text-green-100 text-xs mt-0.5 opacity-90">
                    {otpVerified
                      ? `${otpChannel === 'email' ? otpContact : otpContact} — terms auto-accepted`
                      : 'Enter your email or phone, then verify with a one-time code.'}
                  </p>
                </div>
                {otpVerified && (
                  <button type="button" onClick={clearOtp}
                    className="ml-auto text-xs text-green-200 hover:text-white underline flex-shrink-0">
                    Change
                  </button>
                )}
              </div>
                ...
                {otpVerified ? (
                  ...
                  <div className="flex items-center gap-3 p-3 bg-green-50 border border-green-200 rounded-xl">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold text-green-700">Verified via {otpChannel}</p>
                      <p className="text-xs text-gray-500 truncate">{otpContact}</p>
                    </div>
                    <span className="text-[10px] font-bold text-green-600 bg-green-100 border border-green-300 rounded-full px-2 py-0.5">Terms ✓</span>
                  </div>
                ) : (
                  /* ─ Input + OTP flow ─ */
                  <>
                    ...
                    <div className="flex gap-2">
                      {(['email', 'phone'] as const).map(ch => (
                        <button key={ch} type="button"
                          onClick={() => {
                            if (otpChannel !== ch) {
                              setOtpChannel(ch); setOtpSent(false); setOtpCode(''); setOtpError(null)
                            }
                          }}
                          className={`flex-1 py-1.5 rounded-xl text-xs font-bold border-2 transition-all ${
                            otpChannel === ch
                              ? 'bg-[var(--dict-blue)] border-[var(--dict-blue)] text-white'
                        ...
                        <div className="flex gap-2">
                          {otpChannel === 'email' ? (
                            <input
                              ...
                              onKeyDown={e => e.key === 'Enter' && sendOtp(form.contactEmail)}
                              placeholder="e.g. juandelacruz@gmail.com"
                              maxLength={200}
                              className={`flex-1 border-2 rounded-xl px-3 py-2.5 text-sm outline-none transition-colors ${
                            ...
                            <input
                              ...
                              onKeyDown={e => e.key === 'Enter' && sendOtp(form.contactPhone)}
                          ...
                          <button type="button"
                            onClick={() => sendOtp(otpChannel === 'email' ? form.contactEmail : form.contactPhone)}
                            disabled={otpLoading || !(otpChannel === 'email' ? form.contactEmail : form.contactPhone).trim() || !form.fullName.trim()}
                            className="px-3 py-2.5 bg-[var(--dict-blue)] text-white rounded-xl text-xs font-bold disabled:opacity-50 whitespace-nowrap flex items-center gap-1.5">
                            {otpLoading
                    ...
                    {/* OTP digit boxes — shown after code sent */}
                    {otpSent && (
                      <div className="space-y-3">
                        ...
                        <div className="flex gap-1.5 justify-center">
                          {[0,1,2,3,4,5].map(i => (
                            <input
                              ...
                              className={`w-10 h-12 text-center text-xl font-mono font-bold border-2 rounded-xl outline-none transition-all ${
                        ...
                        <button type="button" onClick={verifyOtp}
                          ...
                          className="w-full py-2.5 bg-green-600 hover:bg-green-700 text-white rounded-xl text-sm font-bold disabled:opacity-50 flex items-center justify-center gap-2 transition-colors">
                          {otpLoading
                        ...
                        <div className="text-center">
                          {otpCountdown > 0
                            ...
                            : <button type="button" onClick={() => { setOtpSent(false); setOtpCode(''); setOtpError(null) }}
                                className="text-xs text-[var(--dict-blue)] underline">Resend or change contact</button>
                          }
                        </div>
                      </div>
                    )}
                    ...
                    <p className="text-[11px] text-gray-400 leading-relaxed">
                      Verifying auto-accepts all terms below. Your contact is stored securely per{' '}
